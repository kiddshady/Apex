...
/* ═══════════════════════════════════════════════════════════════════════════
   APEX — lo que el tracker necesita además del set base.
   Mismas reglas: nada de toFixed, todo por el formateador del locale.
   ═══════════════════════════════════════════════════════════════════════════ */

/** Una cantidad de dosis: 200 · 0,5 · 1500 · 12,25. Sin abreviar en «k» y sin
    agrupar miles: una dosis se lee entera o no se lee. */
...
/** Cantidad con su unidad: «200 mg». La unidad va con espacio fino de por medio. */
export function fmtDosis(n, unidad = '') {
  return unidad ? `${fmtQty(n)} ${unidad}` : fmtQty(n);
}
...
/* Los nombres de días y meses van a mano y no por Intl: Node escribe «05-sept»
   donde Chromium escribe «05 sept», y un test que pase en uno y falle en el otro
   no sirve. Además «sep» de tres letras entra en un eje; «sept» no siempre. */
const MESES = ['ene', 'feb', 'mar', 'abr', 'may', 'jun', 'jul', 'ago', 'sep', 'oct', 'nov', 'dic'];
const MESES_LARGOS = ['enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'];
const DIAS = ['dom', 'lun', 'mar', 'mié', 'jue', 'vie', 'sáb'];
const DIAS_LARGOS = ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'];

export const fmtMes = (i) => MESES[((i % 12) + 12) % 12];
export const fmtDiaSemanaNombre = (i, largo = false) => (largo ? DIAS_LARGOS : DIAS)[((i % 7) + 7) % 7];

/** «05 sep» — para los ejes, donde el año sobra. */
export function fmtDiaCorto(ts) {
  if (!ts) return '—';
  const d = new Date(Number(ts));
  return `${String(d.getDate()).padStart(2, '0')} ${MESES[d.getMonth()]}`;
}

/** «viernes 5 de septiembre» — el encabezado de un día. */
export function fmtDiaLargo(ts) {
  if (!ts) return '—';
  const d = new Date(Number(ts));
  return `${DIAS_LARGOS[d.getDay()]} ${d.getDate()} de ${MESES_LARGOS[d.getMonth()]}`;
}

/** «vie 05 sep» — una fila de lista. Con el año si no es el actual. */
export function fmtDiaSemana(ts) {
  if (!ts) return '—';
  const d = new Date(Number(ts));
  const anio = d.getFullYear() !== new Date().getFullYear() ? ` ${d.getFullYear()}` : '';
  return `${DIAS[d.getDay()]} ${String(d.getDate()).padStart(2, '0')} ${MESES[d.getMonth()]}${anio}`;
}

/** Minutos legibles para las estadísticas: 45 min · 1 h 05 min · 2 h · 3 d 4 h. */
export function fmtMin(min) {
  if (min == null || !Number.isFinite(Number(min))) return '—';
  const m = Math.round(Math.abs(Number(min)));
  const signo = Number(min) < 0 ? '-' : '';
  if (m < 60) return `${signo}${m} min`;
  const h = Math.floor(m / 60);
  const mm = m % 60;
  if (h < 48) return `${signo}${h} h${mm ? ` ${String(mm).padStart(2, '0')} min` : ''}`;
  const d = Math.floor(h / 24);
  return `${signo}${d} d${h % 24 ? ` ${h % 24} h` : ''}`;
}

/** Un desplazamiento desde la toma, compacto: +45m · +1h 05m · +2h · 0m. */
export function fmtOffset(ms, { signo = true } = {}) {
  if (ms == null || !Number.isFinite(Number(ms))) return '—';
  const m = Math.round(Math.abs(Number(ms)) / 60000);
  const pre = Number(ms) < 0 ? '-' : (signo && m > 0 ? '+' : '');
  if (m < 60) return `${pre}${m}m`;
  const h = Math.floor(m / 60);
  const mm = m % 60;
  if (h < 48) return `${pre}${h}h${mm ? ` ${String(mm).padStart(2, '0')}m` : ''}`;
  const d = Math.floor(h / 24);
  return `${pre}${d}d${h % 24 ? ` ${h % 24}h` : ''}`;
}

/** Horas con un decimal, para los ejes del perfil: 1,5 h. */
export function fmtHoras(h) {
  const v = Number(h);
  if (!Number.isFinite(v)) return '—';
  return `${nf(0, 1).format(v)} h`;
}
