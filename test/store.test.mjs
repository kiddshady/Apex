...
const parche = await store.saveSettings({ rangoRegistro: '90d' });
ok('guardar un parche no pisa el resto', parche.rangoRegistro === '90d' && parche.unidadDefault === store.DEFAULT_SETTINGS.unidadDefault);
// Una clave nueva del código tiene que aparecer en un archivo viejo.
await store.writeJSON(store.SETTINGS_FILE, { schema: store.SCHEMA, rangoRegistro: '7d' });
const completado = await store.loadSettings();
ok('las claves nuevas se completan solas', 'unidadDefault' in completado && completado.rangoRegistro === '7d');
